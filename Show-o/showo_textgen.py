...
def generate_text(question, model, uni_prompting, device, config, vq_model, clip_image_processor):
    ...
    max_new_tokens = 512
    config.model.showo.w_clip_vit = False
...
if __name__ == '__main__':
    question = """Which of the following data sets corresponds to an optically active saturated hydrocarbon? Please select the letter corresponding to the answer you believe to be correct from the options below.
(A)%age Composition = C 82.75 % and H 17.25 %
EI-MS: m/z = 58, 43, 27, 15
(B)%age Composition = C 83.33 % and H 16.66 %
EI-MS: m/z = 72, 57, 43, 29
(C)%age Composition = C 83.72 % and H 16.27 %
EI-MS: m/z = 86, 57, 43, 29
(D)%age Composition = C 84 % and H 16 %
EI-MS: m/z = 100, 71, 57, 43"""
    model, uni_prompting, device, config, vq_model, clip_image_processor = get_model()
    answer = generate_text(question, model, uni_prompting, device, config, vq_model, clip_image_processor)
    print(answer)
...
#     images = []
#     config.question = ['What is in this picture?']
...
#             print("config.model.showo.w_clip_vit")
#             print(config.model.showo.w_clip_vit)
...
#                 cont_toks_list = model.mmu_generate(input_ids, attention_mask=attention_mask,
#                                             max_new_tokens=512, top_k=top_k,
#                                             eot_token=uni_prompting.sptids_dict['<|eot|>'])
