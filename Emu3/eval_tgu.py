...
from emu3_textgen import get_model, generate_text
############################################
import os, random
import pandas as pd
...
def main():
    ...
    ############################################
    output_dir = "/hongbojiang/workdirs/TGU/Emu3"
    ############################################
    ...
    for idx in range(sample_num):
        try:
            ...
            if os.path.isfile(save_path):   # 如果存在就跳过
                ...
                continue
            prediction_answer = generate_text(prompt, processor, model, tokenizer)
