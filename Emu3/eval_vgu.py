############################################
import sys
sys.path.append("/hongbojiang/codes/VGU/Emu3")
from emu3_imagegen import get_model, generate_image
############################################
import os, random
import pandas as pd
...
def main():
    ...
    ############################################
    output_dir = "/hongbojiang/workdirs/VGU/Emu3"
    ############################################
    ...
    for idx, prompt_template in enumerate(prompts):
        try:
            ...
            if os.path.isfile(save_path):   # 如果存在就跳过
                ...
                continue
            image_generated = generate_image(prompt, processor, model)
            image_generated.save(save_path)
            ############################################
            print(f"[Rank{os.environ['CUDA_VISIBLE_DEVICES']}] [{idx+1}/{sample_num}]Saved: {save_path}")
