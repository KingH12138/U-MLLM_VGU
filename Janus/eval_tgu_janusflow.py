...
from janusflow_textgen import get_model, get_answer
############################################
import os, random
import pandas as pd
...
def main():
    ############################################
    vl_chat_processor, tokenizer, vl_gpt = get_model()
    ############################################
    # TGU和VGU共用anno
    ############################################
    ...
    ############################################
    output_dir = "/hongbojiang/workdirs/TGU/JanusFlow"
    ############################################
    ...
    for idx in range(sample_num):
        try:
            ...
            if os.path.isfile(save_path):   # 如果存在就跳过
                ...
                continue
            prediction_answer = get_answer(prompt, vl_chat_processor, tokenizer, vl_gpt)
